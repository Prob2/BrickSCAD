...
function row_points(c, l, e, h, is_odd) =
    ...
    is_odd ? [
        ...
        if (c>=2) for (i = [1:c-1]) [brick_width - l/2 + i * l, 0, e+h/2, l, brick_width, h],
    ] : [
        if (c>=2) for (i = [1:c-1]) [-l/2 + i * l, 0, e+h/2, l, brick_width, h],
...
function row_points_symm(c, l, e, h, is_odd) =
    ...
    is_odd ? [
        ...
        if (c>=2) for (i = [1:c-2]) [i * l, 0, e+h/2, l, brick_width, h],
...
function wall_points(length, height) = 
    ...
    let (i_brick_height = height / row_count)
        flatten([
            for (r = [0:row_count-1]) row_points(row_brick_count, i_brick_length, r * i_brick_height, i_brick_height, r%2==1),
        ]);
...
function wall_points_symm(length, height) = 
    ...
    let (i_brick_height = height / row_count)
        flatten([
            for (r = [0:row_count-1]) row_points_symm(row_brick_count, i_brick_length, r * i_brick_height, i_brick_height, r%2==1),
        ]);
